...
# Author:       Joe Lapinski
#
# Purpose:      Student Tracker App. Demonstrating OOP, Tkinter GUI module,
...
def ask_quit(self):
    if messagebox.askokcancel("Exit program", "Okay to exit application?"):
        # This closes app (prevents memory leaks upon closeout)
        self.master.destroy()
        os._exit(0)
...
def create_db(self):
    ...
    with conn:
        cur = conn.cursor()
        # Drop the table if it exists to ensure it has the correct structure
        cur.execute("DROP TABLE IF EXISTS tbl_studentTracker")
...
def first_run(self):
    ...
    with conn:
        ...
        if count < 1:
            cur.execute("""INSERT INTO tbl_studentTracker (col_fname,col_lname,col_fullname,col_phone,col_email,col_course) VALUES (?,?,?,?,?,?)""", ('John','Doe','John Doe','111-111-1111','jdoe@email.com','Computer and Technology Basics'))
            conn.commit()
    conn.close()
...
def onSelect(self,event):
    ...
    with conn:
        cursor = conn.cursor()
        cursor.execute("""SELECT col_fname,col_lname,col_phone,col_email,col_course FROM tbl_studentTracker WHERE col_fullname = (?)""", [value])
...
def addToList(self):
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and(len(var_email) > 0) and (len(var_course) > 0): # enforce the user to provide both names
        ...
        with conn:
            ...
            cursor.execute("""SELECT COUNT(col_fullname) FROM tbl_studentTracker WHERE col_fullname = '{}'""".format(var_fullname))#,(var_fullname))
            count = cursor.fetchone()[0]
            chkName = count
            if chkName == 0: # if this is 0 then there is no existance of the fullname and we can add new data
                print("chkName: {}".format(chkName))
                cursor.execute("""INSERT INTO tbl_studentTracker(col_fname,col_lname,col_fullname,col_phone,col_email,col_course) VALUES (?,?,?,?,?,?)""",(var_fname,var_lname,var_fullname,var_phone,var_email,var_course))
    ...
    with conn:
        ...
        if count > 1:
            ...
            if confirm:
                ...
                with conn:
                    cursor = conn.cursor()
                    cursor.execute("""DELETE FROM tbl_studentTracker WHERE col_fullname = '{}'""".format(var_select))
...
def onRefresh(self):
    ...
    with conn:
        ...
        i = 0
        while i < count:
                cursor.execute("""SELECT col_fullname FROM tbl_studentTracker""")
...
def onUpdate(self):
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0): # ensure that there is data present
        ...
        with conn:
            ...
            cur.execute("""SELECT COUNT(col_phone) FROM tbl_studentTracker WHERE col_phone = '{}'""".format(var_phone))
            count = cur.fetchone()[0]
            print(count)
            cur.execute("""SELECT COUNT(col_email) FROM tbl_studentTracker WHERE col_email = '{}'""".format(var_email))
            ...
            if count == 0 or count2 == 0: # if proposed changes are not already in the database, then proceed
                ...
                if response:
                    ...
                    with conn:
                        cursor = conn.cursor()
                        cursor.execute("""UPDATE tbl_studentTracker SET col_phone = '{0}',col_email = '{1}' WHERE col_fullname = '{2}'""".format(var_phone,var_email,var_value))
                        onClear(self)
                        conn.commit()
                else:
